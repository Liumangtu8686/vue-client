...
onMounted(() => {
  if (!canvas) {
    ...
    img.src = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKCAYAAACNMs+9AAAAFElEQVR42mNk+A8AAQUBAScY42gAAAAASUVORK5CYII=';
  }
});

const isCanvasImage = (text) => {
  // 判断是否是Canvas格式的图片
  return text === 'canvas';
};
...
const parse3DImage = (text) => {  
  const container = document.createElement('div');
  const viewer = document.createElement('model-viewer');
  viewer.setAttribute('src', text);
...
// 重置上下文节点
async function resetParentMessageId() {
  if (!localStorage.getItem("access_token")) return;
  messages.value = [];
  let res = await cloud.invoke("resetParentMessageId");
  return ElMessage({
      message: res,
      type: "info",
    });
...
async function sendStreamMessage() {
 ...
 // 我们提问的内容
  ...
  try {
    ...
    axios({
      url:'https://ztvufn.laf.dev/robot-test',    
      method: 'post',
      data,
      responseType: 'stream',
      timeout: 180000,
...
async function sendNotStreamMessage() {
 ...
 // 我们提问的内容
  ...
  if (message == "") {
    ...
    setScreen();
    return ElMessage({
      message: "quesion isnot be empty",
      type: "error",
    });
  }
  newMessage.value = '';
  const newData = { text: message, type: "sent", index: messages.value.length};
...
<template>
  <div id="background">
    ...
    <div class="login-out" >
      <button class="login-button" @click="loginOut" v-if = "isLoging">login out</button>
      <button class="login-button" @click="loginIn" v-if = "!isLoging">login in</button>
    </div>
    <div class="conversation" ref="chatBox">
      <div class="about"> 
        <div class="input-group" >
          ...
          <select id="preset-text" v-model="newMessage" style="width:calc(100% - 90px);max-width:200px;">
              ...
              <option value="接下来我会给你指令，生成相应的图片，我希望你用Markdown语言生成，不要用反引号，不要用代码框，你需要用Unsplash API，遵循以下的格式：https://source.unsplash.com/1600x900/?< PUT YOUR QUERY HERE >。你明白了吗？">高清图片搜索器</option>
              ...
              <option value="我希望你充当 javascript 控制台。我将键入命令，您将回复 javascript 控制台应显示的内容。我希望您只在一个唯一的代码块内回复终端输出，而不是其他任何内容。不要写解释。除非我指示您这样做。我的第一个命令是 console.log('hello');">充当 JavaScript 控制台</option>
              ...
              <option value="接下来我发送给你的句子，你应尽可能多地使用同义词替换其中的词语，例如避免改为规避，如果改为若是，每个句子必须保证13个字符不能相同，汉字算两个字符，英文单词算一个，不能仅通过删除、增加、修改一两个字符的方式，可以在无法替换的句子中间插入一些无意义又无影响的词语来规避，也可以在不影响其含义的情况下修改语序，可以使用缩写的方式，必须严格遵守这条规则，如果明白了的话请发一条示例吧">简单去重工具</option>
          </select>
        </div>
      </div>
      <div class="message-list" ref="messageList">
        <div v-for="(message, index) in messages" :key="index" :class="['message', message.type]">
          ...
          <div v-if="message.type === 'received' " class="img" style="flex: 1;text-align:left"> 
              ...
              <!-- <div v-else-if="isCanvasImage(message.contentType)">
      ...
      <div class="input-box">
        <input type="text" v-model="newMessage" class="todo-input" placeholder="Type a message..." @keyup.enter="sendMessage"/>
        <button class="todo-button" @click="sendMessage">send</button>
      </div>
    </div>
  </div>
</template>
...
.res-button{
  ...
  color: #9ca2a8;
  float: left;
  margin-right: 10px; /* 可选: 添加一些右边距，以防止按钮与文本之间的内容过于接近 */
}
...
.todo-judgbutton:hover { transform: translateY(-2px); box-shadow: 0 4px 8px rgba(0, 0, 0, 0.3); }

.todo-judgbutton:active { transform: translateY(2px); box-shadow: none; }


.todo-button:hover { transform: translateY(-2px); box-shadow: 0 4px 8px rgba(0, 0, 0, 0.3); }

.todo-button:active { transform: translateY(2px); box-shadow: none; }
